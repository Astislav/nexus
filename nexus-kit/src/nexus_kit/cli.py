...
by `nexus-kit guides`). Read AGENTS.md and the map it references before touching
...
to when relevant — don't load them all.

**Read `.nexus-kit/map.md`** for the nexus-kit guides.

<!-- Claude Code: to auto-import the map into context (instead of reading it on
     request), add this line here — @.nexus-kit/map.md -->

<!-- After `uv add`-ing, upgrading or removing any nexus-kit package, run
     `uv run nexus-kit guides` to refresh `.nexus-kit/`. Commit `.nexus-kit/`:
     reviewing its diff is how you vet what your assistant will read. -->
""",
    ".nexus-kit/map.md": """\
...
#   - The atlas is committed, so a new/changed guide shows up in `git diff`. That
#     review is the trust boundary: it makes a malicious guide VISIBLE, not
#     blocked — the entry point stops accidental/transitive guides, but a package
#     you deliberately install and commit is on you. No allow-list, no quarantine.
...
def _build_guides(check: bool) -> None:
    ...
    if mounted:
        ...
        print("  (e.g. `Read .nexus-kit/map.md`; Claude Code users can `@.nexus-kit/map.md`).")
        print("  We never edit AGENTS.md/CLAUDE.md — that's yours. `nexus-kit new` sets it up for fresh apps.")
    print("  Commit .nexus-kit/ and review its diff — that review is what vets a guide's contents.")
